...
DCL_LOG_PATTERN += ANY_CHARS_REGEX          # followed by text
...
ENDING_TIMESTAMP_PATTERN = TIMESTAMP + MULTI_SPACE  # DCL controller timestamp
ENDING_TIMESTAMP_PATTERN += END_OF_LINE_REGEX
ENDING_TIMESTAMP_MATCHER = re.compile(ENDING_TIMESTAMP_PATTERN)
...
class AdcptAcfgmPd8Parser(SimpleParser):
    ...
    def parse_file(self):
        ...
        while line:
            ...
            if dcl_log_match is not None:
                # Discard unneeded data, move on to the next line
                line = self._stream_handle.readline()
                continue
            ...
            if line_match is None:
                ...
                parsed_data.append(matches[SENSOR_GROUP_TIMESTAMP])
                        ...
                        if line_match is None:
                            ...
                            while True:
                                ...
                                if line_match is not None:
                                    ...
                                    line_match = ENDING_TIMESTAMP_MATCHER.match(line)
                                    if line_match is None:
                                        self.recov_exception("Expecting ending DCL Timestamp, "
                                                             "received: %r" % line)
                                        break
                                    else:
                                        particle_data = []
                                        ...
                                        particle_data[:0] = line_match.groups()
